/****************MP11: by djb6; partners: tylerjn2 & pdh2***************
*The first function we implemented  is is_leaf_node. We check if the node has any branches by checking the
* left side. Since all nodes have either no child nodes, or two, we can deduce that if it has a left child node,
* it has two subnodes, and none if the left subnode points to NULL. Next we have the is_internal_node that checks
* if the node is in the middle of the branch, and returns 1. If both of its child node pointers point to NULL, it is
* not internal, and the function returns 0, otherwise it returns 1. The next function is is_in_subtree.This Function
* recursively checks two nodes to see if they are in the same subtree. The base case is if they are equal, and the
* function recursively calls itself using the nodes' child nodes as inputs. The next function, rotate, switches the
* heigh and width pointers. The next function recut, checks the cutline, and then switches a verticl cutline with a
* horizontal one and vice versa. The swap_module function swaps the module pointers of two nodes. swap_topology is used
* to swap the location of two nodes in the tree. This is done by using temporary pointers, swapping the left or right
* child pointer of the parent nodes, and then setting the parent pointer of the swapped nodes to their new parents.The
* get_expression function clears the expression, and then calls postfix_traversal in order to get the final postfix
* expression. postfix_traversal operates by recursively calling itself down the tree in orer to create the postfix
* expression for the tree. The total resources required is gathered by iterating through the tree and summing
* the cost of the modules. Finally the init_slicing_tree function recursively calls itself in order to
...
int is_leaf_node(node_t* ptr) {
  ...
  //Acessing within node_t the NODE*left and NODE*right and checking if the those
  // values are NULL?
      if(ptr->left==NULL){      //it has to have two leaves, so I think we just have to check for one child???
          return 1;     //return 1 if given node is a leaf node
      }else{
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  //has child nodes
      if(ptr->left != NULL && ptr->right != NULL){    //if given node has two child nodes
          return 1;     //return 1 if given node is an internal node
      }else{
          return 0;   //otherwise return 0
      }
...
int is_in_subtree(node_t* a, node_t* b) {
        ...
        if(b==NULL){      //if reached the end of the tree, it's not in it
          return 0;
        }
        if(a==NULL){      //if reached the end of the tree, it's not in it
          return 0;
        }

        if(a==b){       //if they're the same node, it's in the same tree
          return 1;
        }
        //recursive part: check the left and right children of a, thus going down the whole tree until NULL
...
void rotate(node_t* ptr) {
  // TODO:
  if(ptr == NULL){      //if ptr doesn't exist exit
    return;
  }

  int tempval = ptr->module->w;     //set a temp val to swap
  ptr->module->w=ptr->module->h;    //swap width and height
  ptr->module->h=tempval;
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline == V){      //swap the horizontal and vertical cutlines
    ptr->cutline = H;         //by checking which one it is and setting
  }else{                      //it as the other one
    ptr->cutline = V;
  }
...
void swap_module(node_t* a, node_t* b) {
  ...
  node_t * tempA = (node_t*)malloc(sizeof(node_t)); //allocate memory for a temporary node for swapping
  tempA->module = a->module;        //set the temporary module to the module in a
  a->module = b->module;            //swap the two modules
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
        node_t * atemp = (node_t*)malloc(sizeof(node_t));    //set two temporary nodes for a and b
        atemp = a->parent;
        node_t * btemp = (node_t*)malloc(sizeof(node_t));
        btemp = b->parent;
        node_t * aclone = (node_t*)malloc(sizeof(node_t));  //make a node clone of a
        aclone = a;

        if(atemp->right == a){          //check if a is the right child
            (a->parent)->right = b;      //swap the right child
            b->parent = a->parent;        //set b's parent to a's parent
        }else if(atemp->left == a){   //check if b is the left child
            (a->parent)->left = b;    //swap the left child
            b->parent = a->parent;      //set b's parent to a's parent
        }

        if(btemp->right == b){      //check if b is the right child
            btemp->right = aclone;    //swap the right child
            a->parent = btemp;      //set a's parent to b's parent

        }else if(btemp->left == b){//check if b is the left child
            btemp->left = aclone; //swap the left child
            a->parent = btemp;  //set a's parent to b's parent
        }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
    ...
    postfix_traversal(ptr->left,nth,expression);    //traverse left tree by recursion
    postfix_traversal(ptr->right,nth,expression);   //traverse right tree by recursion
    if(is_leaf_node(ptr)==1){       //if it's a leaf node
        ptr->cutline = UNDEFINED_CUTLINE;      //remove cutline
        expression[*nth].module = ptr->module;    //set expression to the module
        *nth += 1;      //increment nth
    }else{
        ptr->module = NULL;       //remove module
        expression[*nth].cutline = ptr->cutline;    //set expression to the cutline
        *nth += 1;    //increment nth
    }
...
{
      // TODO:
      int sum = 0;          //initalize sum variable
      for(int i=0;i<num_modules;i++){     //loop through all the modules
        sum = sum + modules[i].resource;    //increment sum by the current module resource
      }
...
node_t* init_slicing_tree(node_t* par, int n) {
      ...
      // TODO:
      node_t * ptr = (node_t*)malloc(sizeof(node_t));//ptr <- malloc a new node of node_t
      ptr->parent = par;    //set the pointer's parent to the function's argument parent
      ptr->right = NULL;      //sets right and left children to NULL
      ptr->left = NULL;

      // Base case
      if(n == num_modules - 1) {     //check if it's reached end of modules
        ptr->module = &modules[n];        //set module to the module value at the current index
        ptr->right = NULL;      //set right and left children to NULL
        ptr->left = NULL;
        ptr->cutline = UNDEFINED_CUTLINE;        //remove cutline as it's a leave node
        return ptr;
      }

      // Internal node
      // assign the data to the module pointed by ptr
      ptr->module = NULL;     //remove nodule as it's an internal node
      ptr->cutline = V;       //set cutline to vertical


      // Right child
      node_t * rightptr = (node_t*)malloc(sizeof(node_t)); //malloc a new node of node_t as the right child of ptr
      ptr->right = rightptr;      //set the right child of the pointer we created to the memory of new node we made
      (ptr->right)->cutline = UNDEFINED_CUTLINE;    //remove cutline at node
      (ptr->right)->module = &modules[n];    //set module to the module value at the current index


      // Left child (recursive call)
      ptr->left = init_slicing_tree(ptr, n+1); //traverse through tree horizontally to the left through recursion

      return ptr;     //return the pointer to the node
